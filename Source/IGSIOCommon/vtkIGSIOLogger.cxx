...
vtkIGSIOLogger* vtkIGSIOLogger::Instance()
{
  if (m_pInstance == NULL)
  {
    ...
    //vtkIGSIOConfig::GetInstance(); // set the log file name from the XML config
    //std::string strPlusLibVersion = std::string("Software version: ") +
    //                                igsioCommon::GetPlusLibVersionString();

#ifdef _DEBUG
    strPlusLibVersion += " (debug build)";
#endif

    //m_pInstance->LogMessage(LOG_LEVEL_INFO, strPlusLibVersion.c_str(), "vtkIGSIOLogger", __LINE__);
  }
