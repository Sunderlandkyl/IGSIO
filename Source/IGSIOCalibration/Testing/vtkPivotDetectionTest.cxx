...
#include "vtkIGSIOPhantomLandmarkRegistrationAlgo.h"
...
{ 
  ...
  if (PivotDetection->ReadConfiguration(configRootElement) != IGSIO_SUCCESS)
  {
  ...
  if( vtkIGSIOSequenceIO::Read(inputTrackedStylusTipSequenceMetafile, trackedStylusTipFrames) != IGSIO_SUCCESS )
  {
  ...
  if ( transformRepositoryCalibration->ReadConfiguration(configRootElement) != IGSIO_SUCCESS )
  {
  ...
  if (valid)
  {
    ...
    for (int i=0; i < trackedStylusTipFrames->GetNumberOfTrackedFrames(); ++i)
    {
      ...
      if ( transformRepository->SetTransforms(*(trackedStylusTipFrames->GetTrackedFrame(i))) != IGSIO_SUCCESS )
      {
      ...
      valid = false;
      if ( (transformRepository->GetTransform(stylusToReferenceTransformName, stylusToReferenceMatrix, &valid) != IGSIO_SUCCESS) || (!valid) )
      {
  ...
  if (!inputBaselineFileName.empty())
  {
    ...
    vtkSmartPointer<vtkIGSIOPhantomLandmarkRegistrationAlgo> pivotDetectionBaselineLandmarks = vtkSmartPointer<vtkIGSIOPhantomLandmarkRegistrationAlgo>::New();
    if (pivotDetectionBaselineLandmarks == NULL)
    {
      LOG_ERROR("Unable to instantiate phantom registration algorithm class!");
      exit(EXIT_FAILURE);
    }
    if (pivotDetectionBaselineLandmarks->ReadConfiguration(baselineRootElem) != IGSIO_SUCCESS)
    {
